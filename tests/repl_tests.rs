...
fn repl_highlighter_detects_multiline_input() {
    assert!(rask::repl::highlighter::needs_more_input("if true {"));
    assert!(rask::repl::highlighter::needs_more_input("x = (1 + 2"));
    assert!(!rask::repl::highlighter::needs_more_input("x = 1 + 2"));
}
...
fn repl_completion_suggests_keywords_and_symbols() {
    ...
    let completions = rask::repl::highlighter::complete("he", &symbols);
